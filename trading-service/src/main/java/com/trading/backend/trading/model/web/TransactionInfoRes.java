...
public class TransactionInfoRes {
	...
	@ApiModelProperty(value = "订单类型:LIMIT 限价,MARKET 市价,STOP_LIMIT 触发限价,STOP_MARKET 触发市价", notes = "LIMIT,MARKET,STOP_LIMIT,STOP_MARKET",
            example = "LIMIT")
	...
	@ApiModelProperty(value = "操作类型: 杠杆（ OPEN_POSITION 开仓 ADD_POSITION 加仓 CLOSE_POSITION 平仓 REDUCE_POSITION 减仓 SETTLE_POSITION 交割 FORCE_CLOSE 强减发生（包括强平））; " +
			"现货（ PLACED_BY_CLIENT 用户主动发生（前端忽视，展示 - ） AUTO_CONVERSION 自动转换（盈亏转换发生） LIQUIDATION 余额清算 EARN_LIQUIDATION 理财清算 PLACED_BY_API " +
            "API用户发生（前端忽视，展示 - ） " +
			"OTC_SHOP BOOKING用户发生 " +
			" LOAN_LIDQUIDATION 借贷清算 REPAY_WITH_COLLATERAL 借贷质押还款）", example = "USD")
